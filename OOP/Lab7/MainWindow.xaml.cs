...
namespace Lab7
{
    ...
    public partial class MainWindow : Window
    {
            ...
            return
               "use University; " +
                "insert into Student(NAME, SPECIALITY, AGE, BIRTHDAY, COURSE, SEX, AVGSCORE, FOTO) values " +
                    "('Шутро Александр Сергеевич', 'Программное обеспечение информационных технологий', 23, '04-06-2000', 2, 'м', 8.0, null), " +
                    "('Рутковский Глеб Викторович', 'Программное обеспечение информационных технологий', 20, '10-07-1998', 2, 'ж', 8.0, null), " +
                    "('Селех Дмитрий Сергеевич', 'Информационные системы и технологии', 19, '12-07-1997', 3, 'м', 7.0, null), " +
                    "('Авласенко Георгий Александрович', 'Дизайн электронных и веб-изданий', 18, '12-02-2000', 2, 'ж', 7.5, null) " +
        ...
        public MainWindow()
        {
            ...
            string connectionString0 = @"Data Source=DESKTOP-8DLE2VK; Integrated Security=True";
        ...
        {
            if (this.StudentGrid.SelectedItems != null)
            {
                for (int i = this.StudentGrid.SelectedItems.Count - 1; i >= 0; i--)
                {
                    if (this.StudentGrid.SelectedItems[i] is DataRowView datarowView)
                    {
        ...
        private void AddButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            StudentGrid.CanUserAddRows = true;
        }
        ...
        private void MainTab_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                ...
                if (this.currentTableName != item.Header.ToString())
                {
                    ...
                    if (this.currentTableName == Tables.Student.ToString())
                    {
                        StudentGrid = this.StudentGrid;
                        this.currentPrimaryColumnName = PrimaryColumn.ID.ToString();
                    }
                    if (this.currentTableName == Tables.Address.ToString())
                    {
                        StudentGrid = this.AddressGrid;
                        this.currentPrimaryColumnName = PrimaryColumn.Id.ToString();
                    }
                    this.pageNumber = 0;
                    FillCurrentGrid();
        ...
        private void FillCurrentGrid()
        {
            using (SqlConnection connection = new SqlConnection(this.connectionString))
            {
                ...
                StudentGrid.ItemsSource = this.ds.Tables[0].DefaultView;
